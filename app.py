...
@app.route('/CalendarView/<holidays_number>/<year>/<country>/<state>/<max_workdays>/<min_holidays>/<weekend_days>')
def calendar_view(holidays_number, year, country, state, max_workdays, min_holidays, weekend_days):
    _year= int(year)
    ...
    holiday_planner = HolidayPlanner(int(holidays_number),country, state, int(_year), max_workdays, min_holidays, weekend_days)
